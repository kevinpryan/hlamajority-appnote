...
mutation.df.nras.without.q61h <- data.frame(
  gene = "NRAS",
  mutation = c(
    "Q61R",
    #"Q61H",
...
mutation.df.combined.without.q61h <- rbind(mutation.df, mutation.df.nras.without.q61h)
...
samples2 <- data.frame(sample = rep("all_samples", nrow(mutation.df.combined.without.q61h)))
...
mutation.df.without.nras.q61h.combined.samples <- cbind.data.frame(samples2, mutation.df.combined.without.q61h)
write.csv(mutation.df.combined.without.q61h, file = "../../data/processed/neoantigen-prediction/mutations-without-nras-q61h.csv", row.names = F, quote = F)
