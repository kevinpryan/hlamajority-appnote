...
#   labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-full")
# )
...
df$Tool <- gsub("hlamajority", "nf-hlamajority-full", x = df$Tool)
# df$Tool <- factor(
#   df$Tool,
#   levels = c("kourami", "hlala", "polysolver", "optitype", "hlamajority"),
#   labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-full")
...
# read in majority vote without polysolver
all.in.no.polysolver <- read.table("../../data/processed/1000-genomes/majority/without_polysolver_v2/nf_hlamajority_all_calls.tsv", sep = "\t", header = T)
results.no.polysolver <- readRDS("../../data/processed/1000-genomes/majority/without_polysolver_v2/1000-genomes-full-results-hlamajority-majority-vote-without-polysolver.Rds")
# results.no.polysolver$summary$Tool <- factor(
#   results.no.polysolver$summary$Tool,
#   levels = c("kourami", "hlala", "optitype", "hlamajority"),
#   labels = c("Kourami", "HLA*LA", "Optitype", "nf-hlamajority-without-Polysolver")
# )
results.no.polysolver$summary$Tool <- gsub("hlamajority", "nf-hlamajority-reduced", x = results.no.polysolver$summary$Tool)

df.no.polysolver <- read.csv("../../data/processed/1000-genomes/majority/without_polysolver_v2/1000-genomes-full-stats-hlamajority-majority-vote-without-polysolver.csv")
df.no.polysolver <- df.no.polysolver %>% dplyr::mutate(Accuracy = 100*Accuracy)
df.no.polysolver$Gene <- factor(df.no.polysolver$Gene, levels = c("Overall", "A", "B", "C"))
# df.no.polysolver$Tool <- factor(
#   df.no.polysolver$Tool,
#   levels = c("kourami", "hlala", "polysolver", "optitype", "hlamajority"),
#   labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-without-polysolver")
# )
df.no.polysolver$Tool <- gsub("hlamajority", "nf-hlamajority-reduced", x = df.no.polysolver$Tool)

df.no.polysolver.for.bind <- df.no.polysolver %>% dplyr::filter(Tool == "nf-hlamajority-reduced")
df.combined <- rbind.data.frame(df, df.no.polysolver.for.bind)
df.combined$Tool <- factor(
  df.combined$Tool,
  levels = c("kourami", "hlala", "polysolver", "optitype", "nf-hlamajority-reduced", "nf-hlamajority-full"),
  labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-reduced", "nf-hlamajority-full")
)
my_gene_labels <- c(
...
best_wes <- df.combined %>% dplyr::filter( !(Tool == "nf-hlamajority-reduced" | Tool == "nf-hlamajority-full") & Gene != "Overall") %>% group_by(Gene) %>% dplyr::filter(Accuracy == max(Accuracy)) %>% dplyr::select(Gene, Tool, Accuracy) %>% mutate(Tool = "best_wes") 
meta_wes <- df %>% dplyr::filter(Tool == "nf-hlamajority-full" & Gene != "Overall") %>%  dplyr::select(Gene, Tool, Accuracy)

best_wes$Tool <- "best_wes"
meta_wes$Tool <- "meta_wes"
full_results <- rbind.data.frame(best_wes, meta_wes)
full_results$Method <- "nf-hlamajority-full"
...
df.for.comparison$Tool <- gsub("nf-hlamajority-full", "Metaclassifier", df.for.comparison$Tool)
...
df.for.comparison$Tool <- factor(
  df.for.comparison$Tool,
  levels = c("kourami", "hlala", "polysolver", "optitype", "Metaclassifier"),
  labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "Metaclassifier")
)
...
figureD <- ggplot(df.for.figureD, aes(x = Tool, y = Accuracy, fill = Tool)) +
  ...
  scale_fill_manual(values = c("#999999", "#999999", "#999999", "#999999", "#999999", "#E69F00")) +
...
# get nf-hlamajority reduced
# all.in.no.polysolver.cell.lines <- read.table("../../data/processed/cell-lines-after-polysolver-change/majority/without_polysolver/nf_hlamajority_all_calls.tsv", sep = "\t", header = T)
# results.no.polysolver.cell.lines <- readRDS("../../data/processed/cell-lines-after-polysolver-change/majority/without_polysolver/nci-full-results-hlamajority-majority-vote-without-polysolver.Rds")
df.no.polysolver.cell.lines <- read.csv("../../data/processed/cell-lines-after-polysolver-change/majority/without_polysolver/nci-full-stats-hlamajority-majority-vote-without-polysolver.csv")
df.no.polysolver.cell.lines <- df.no.polysolver.cell.lines %>% dplyr::mutate(Accuracy = 100*Accuracy)
df.no.polysolver.cell.lines.for.bind <- df.no.polysolver.cell.lines %>% dplyr::filter(Tool == "hlamajority")
df.no.polysolver.cell.lines.for.bind$Tool <- gsub(pattern = "hlamajority", replacement = "nf-hlamajority-reduced", x = df.no.polysolver.cell.lines.for.bind$Tool)
df.cell.lines.for.plot <- rbind.data.frame(df, df.no.polysolver.cell.lines.for.bind)
df.cell.lines.for.plot$Tool <- factor(
  df.cell.lines.for.plot$Tool,
  levels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-reduced", "nf-hlamajority-full"),
  labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-reduced", "nf-hlamajority-full")
)
my_gene_labels <- c(
  "Overall" = "Overall Accuracy",
...
cell.line.performance.after.polysolver.change <- ggplot(df.cell.lines.for.plot, aes(x = Tool, y = Accuracy, fill = Tool)) +
...
figureD_figureE
ggsave(plot = figureD_figureE, filename = "/hlamajority-paper/results/app_note/plots/hlamajority-1000genomes-nci-combined-20260915.svg", width = 20, height = 7)
