...
df$Tool <- gsub(pattern = "hlamajority", replacement = "nf-hlamajority-full", x = df$Tool)
df$Tool <- factor(
  df$Tool,
  levels = c("kourami", "hlala", "polysolver", "optitype", "nf-hlamajority-full"),
  labels = c("Kourami", "HLA*LA", "Polysolver", "Optitype", "nf-hlamajority-full")
)

df.without.polysolver <- read.csv("../../data/processed/1000-genomes/wgs-30x-149samples-majority/without_polysolver/1000-genomes-30x-wgs-full-stats-hlamajority-majority-vote-without-polysolver.csv")
df.without.polysolver <- df.without.polysolver %>% dplyr::mutate(Accuracy = 100*Accuracy)
df.without.polysolver.for.bind <- df.without.polysolver %>% dplyr::filter(Tool == "hlamajority")
df.without.polysolver.for.bind$Tool <- gsub(pattern = "hlamajority", replacement = "nf-hlamajority-reduced", x = df.without.polysolver.for.bind$Tool)

df.for.plotting <- rbind.data.frame(df, df.without.polysolver.for.bind)
df.for.plotting$Tool <- factor(
  df.for.plotting$Tool,
...
df.for.plotting$Study <- "nf-hlamajority"
df.for.plotting$Accuracy <- round(df.for.plotting$Accuracy, 1)
...
figure <- ggplot(df.for.plotting, aes(x = Tool, y = Accuracy, fill = Tool)) +
